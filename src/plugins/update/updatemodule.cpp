    ...
    connect(m_model, &UpdateModel::updateNotifyChanged, this, [this](const bool state) {
        ...
        if (!state) {
            ...
            if (status == UpdatesStatus::UpdatesAvailable || status == UpdatesStatus::Updating || status == UpdatesStatus::Downloading || status == UpdatesStatus::DownloadPaused || status == UpdatesStatus::Downloaded ||
                    status == UpdatesStatus::Installing || status == UpdatesStatus::RecoveryBackingUp || status == UpdatesStatus::RecoveryBackupSuccessfully || m_model->getUpdatablePackages()) {
...
void UpdateModule::notifyDisplayReminder(UpdatesStatus status)
{
    ...
    if (status == UpdatesStatus::Checking) {
        //do nothing
               ...
               status == UpdatesStatus::RecoveryBackupSuccessfully) {
        m_frameProxy->setModuleSubscriptVisible(name(), true);
    } else {
        m_frameProxy->setModuleSubscriptVisible(name(), false);
    }
